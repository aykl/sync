...
logViewer =
  R.div
    [ RP._id "acp-logview", RP.className "acp-panel col-md-12"
    ...
    , R.div [ RP.className "input-group" ]
        [ R.div [ RP.className "input-group-btn" ]
            [ R.button
                [ RP._id "acp-syslog-btn", RP.className "btn.btn-default" ]
                [ R.text "Syslog" ]
            , R.button
                [ RP._id "acp-errlog-btn", RP.className "btn.btn-default" ]
                [ R.text "Error log" ]
            , R.button
                [ RP._id "acp-httplog-btn", RP.className "btn.btn-default" ]
                [ R.text "HTTP log" ]
            ]
        , R.input
            [ RP._id "acp-chanlog-name"
    ...
    , R.div'
        [ R.form
            ...
            [ R.div [ RP.className "form-group" ]
                [ R.label
                    [ RP.className "control-label.col-sm-2"
                    , RP.htmlFor "acp-announce-title"
                    ]
                    [ R.text "Title" ]
                , R.div [ RP.className "col-sm-10" ]
                    [ R.input
                        [ RP._id "acp-announce-title"
                        , RP.className "form-control"
                        , RP._type "text"
                        ] []
                    ]
  ...
  R.div
    ...
    , R.div [ RP.style {"max-width": "50%"} ]
        [ R.form
            ...
            [ R.div [ RP.className "form-group" ]
                [ R.label
                    [ RP.className "control-label col-sm-3"
...
users =
  R.div
    ...
    , R.div [ RP.className "input-group", RP.style {"max-width": "25%"} ]
        [ R.input
            [ RP._id "acp-ulookup-name"
            , RP.className "form-control"
            , RP._type "text"
            ] []
        , R.span [ RP.className "input-group-btn" ]
            [ R.button
                [ RP._id "acp-ulookup-btn", RP.className "btn btn-default" ]
                [ R.text "Search" ]
            ]
        ]
    , R.table
        [ RP.className "table table-bordered table-striped"
        , RP.style {"margin-top": "20px"}
        ]
        [ R.thead'
            [ R.tr'
                [ R.th [ RP.className "sort", RP._data {"key": "id"} ]
                    [ R.text "ID" ]
                , R.th [ RP.className "sort", RP._data {"key": "name"} ]
                    [ R.text "Name" ]
                , R.th [ RP.className "sort", RP._data {"key": "global_rank"} ]
                    [ R.text "Rank" ]
                , R.th [ RP.className "sort", RP._data {"key": "email"} ]
                    [ R.text "Email" ]
                , R.th' [ R.text "Actions" ]
                ]
            ]
...
channels =
  R.div
    ...
    , R.form
        ...
        , R.div [ RP.className "form-group" ]
            [ R.select
                [ RP._id "acp-clookup-field", RP.className "form-control" ]
                [ R.option [ RP.value "name" ] [ R.text "Channel Name" ]
                , R.option [ RP.value "owner" ] [ R.text "Channel Owner" ]
                ]
            ]
        , R.button
            [ RP._id "acp-clookup-submit"
    ...
    , R.table
        ...
        [ R.thead'
            [ R.tr'
                ...
                , R.th [ RP.className "sort", RP._data {"key": "name"} ]
                    [ R.text "Name" ]
                , R.th [ RP.className "sort", RP._data {"key": "owner"} ]
                    [ R.text "Owner" ]
                , R.th' [ R.text "Control" ]
                ]
            ]
...
eventLog =
  R.div
    ...
    , R.select
        [ RP._id "acp-eventlog-filter"
        , RP.className "form-control"
        , RP.multiple "multiple"
        , RP.style {"max-width": "25%"}
        ] []
    , R.button
        [ RP._id "acp-eventlog-refresh"
        , RP.className "btn btn-default"
        ]
        [ R.text "Refresh" ]
